...
export function PluginOptionsForm({ pluginId, options, initialValues = {} }: PluginOptionsFormProps) {
      ...
      {Object.entries(options).map(([key, option]) => (
        <div key={key} className="space-y-2">
          ...
          {option.field.description && (
            <p className="text-sm text-muted-foreground">{option.field.description}</p>
          )}
          {option.field.validation?.message && values[key] && !new RegExp(option.field.validation.pattern!).test(values[key]) && (
            <p className="text-sm text-destructive">{option.field.validation.message}</p>
          )}
        </div>
      ))}
